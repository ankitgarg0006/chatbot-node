...
/*
  ...
  var endSeq="pdf";
var textcopy=text;
  var startIndex=textcopy.indexOf(startSeq);
  textcopy=textcopy.substr(startIndex);
  var endIndex=textcopy.indexOf(endSeq);

  while(startIndex!=-1)
  {

    var url= textcopy.substr(startIndex,endIndex+endSeq.length);
 textcopy=textcopy.replace(url,'<iframe src="http://docs.google.com/gview?url=' + url + '&embedded=true" style="width:200px; height:200px;" frameborder="0"></iframe> ');
 startIndex=endIndex+endSeq.length;
 textcopy=textcopy.substr(startIndex);
 startIndex=textcopy.indexOf(startSeq);
 endIndex=textcopy.indexOf(endSeq);
...
var textcopy=text;
  ...
  return   textcopy.replace(urlRegex, function(url) {
        console.log("URL", url);
        return '<iframe src="http://docs.google.com/gview?url=' + url + '&embedded=true" style="width:200px; height:200px;" frameborder="0"></iframe> ';
...
module$contents$google3$third_party$apiai$ui$legacy$src$main$webapp$js$agentDemoApp$DomHelper_DomHelper.prototype.setContentOnNode = function(a, b,responseText) {
//const formatted = a.replace(/'asrsti/gi, ' <br /> ')
 a=htmlDecode(a);
 console.log(a);

 //a=responseText.result.speech;
// console.log("GET LINK: ", getlink);
var indexOfCodeCharacter= a.indexOf("    ");
while(indexOfCodeCharacter!=-1)
{

   a=a.replace("    "," <br> ");
  indexOfCodeCharacter= a.indexOf("    ");
}
a = urlify(a);
...
// console.log("AAAAA", a);
console.log("function for setting chatbot content from server response");
  b.innerHTML = a;
  // b.innerHTML = getlink;
  // b.innerHTML+=' <iframe src="http://docs.google.com/gview?url=http://www.africau.edu/images/default/sample.pdf&embedded=true" style="width:200px; height:200px;" frameborder="0"></iframe> ';
  b.className="server-response";
  return this
};
...
var module$contents$google3$third_party$apiai$ui$legacy$src$main$webapp$js$agentDemoApp$App_App = function(a) {
  ...
  //this.sessionId = "tbsceNPWHGdG3luPEBC0wQMWDjT2";
...
module$contents$google3$third_party$apiai$ui$legacy$src$main$webapp$js$agentDemoApp$App_App.prototype.handleResponse = function(a, b) {
  ...
  var responseText=a.responseText;
  a = null;
  try {
    a = JSON.parse(c)
  } catch (d) {
  ...
  this.domHelper.setContentOnNode(c, b,responseText).scrollResultWrapperNodeToBottom()
};
...
module$contents$google3$third_party$apiai$ui$legacy$src$main$webapp$js$agentDemoApp$App_App.API_URL = "https://console.dialogflow.com/api-client/demo/embedded/b6d939fb-55dd-43e9-a30a-52ec67ade224" + "/demoQuery";
